![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript)

![Tailwind](https://img.shields.io/badge/TailwindCSS-06B6D4?style=for-the-badge&logo=tailwind-css)

![AI](https://img.shields.io/badge/Generative_AI-FF6F00?style=for-the-badge)

![OCR](https://img.shields.io/badge/OCR-AI-blue?style=for-the-badge)

![Healthcare](https://img.shields.io/badge/Healthcare-AI-success?style=for-the-badge)

![Privacy](https://img.shields.io/badge/Privacy-First-success?style=for-the-badge)



# 🩺 HealthInsight AI

An AI-powered healthcare application that converts complex medical reports into easy-to-understand insights using OCR, Generative AI, and conversational AI.

Designed with a **Privacy First** approach, HealthInsight AI helps users understand medical reports without replacing professional medical advice.

---

# 🚀 Live Demo

🔗 https://healthinsightai.lovable.app

---

# 📌 Why I Built This

Medical reports often contain complex terminology and laboratory values that are difficult for non-medical users to understand.

Patients frequently search the internet for explanations, which can lead to confusion or inaccurate interpretations.

HealthInsight AI was built to simplify this experience by transforming medical reports into clear, structured, and educational insights while maintaining user privacy.

---

# 🎯 Problem Statement

Understanding health reports requires medical knowledge.

Common challenges include:

- Medical terminology
- Complex laboratory values
- Understanding abnormal findings
- Identifying potential health risks
- Preparing meaningful questions for doctors

---

# 💡 Solution

HealthInsight AI analyzes uploaded reports and provides:

- Executive Summary
- AI-generated Insights
- Risk Assessment
- Abnormal Value Detection
- Lifestyle Recommendations
- Questions to Ask Your Doctor
- Interactive AI Chat Assistant

The application is designed strictly for educational purposes and does not replace licensed healthcare professionals.

---

# ✨ Key Features

✅ Upload PDF, JPG, PNG medical reports

✅ OCR-based report extraction

✅ AI-powered medical interpretation

✅ Executive Summary

✅ Abnormal value detection

✅ Health risk scoring

✅ Lifestyle recommendations

✅ Personalized AI chat

✅ Privacy-first design

✅ No login required

---

# 🏥 Who Can Use This?

- Individuals reviewing health checkups
- Families helping parents understand reports
- Healthcare students
- Recruiters evaluating AI portfolio projects
- Developers exploring AI healthcare solutions

---

# 🏗 System Architecture

```
Medical Report

        │

        ▼

OCR Processing

        │

        ▼

Text Extraction

        │

        ▼

Generative AI Analysis

        │

        ▼

Executive Summary

        │

        ├─────────────► Risk Detection

        │

        ├─────────────► Key Findings

        │

        ├─────────────► Lifestyle Suggestions

        │

        ├─────────────► Questions for Doctor

        │

        ▼

AI Chat Assistant
```

---

# 🔄 Application Workflow

```
Upload Report

↓

OCR Extraction

↓

AI Processing

↓

Medical Summary

↓

Risk Analysis

↓

Recommendations

↓

Interactive AI Chat
```

---

# 📊 Sample Analysis Output

HealthInsight AI generates:

- Executive Summary
- Key Findings
- Critical Alerts
- Risk Score
- Wellness Score
- Specialist Recommendations
- Lifestyle Advice
- Questions for Doctor
- AI Chat Responses

---

# 🔐 Privacy First

HealthInsight AI follows a Privacy-by-Design architecture.

✔ No Login Required

✔ No Account Required

✔ No Personal Data Collection

✔ Reports processed temporarily

✔ Files removed after session

✔ Educational use only

---

# 🧠 AI Chat Assistant

The built-in conversational assistant allows users to ask questions such as:

- What is my most concerning result?
- Should I worry about cholesterol?
- What does Vitamin D deficiency mean?
- Explain my liver function values.
- What questions should I ask my doctor?

---

# ⚙ Tech Stack

Frontend

- React
- TypeScript
- Tailwind CSS
- Vite

AI

- Generative AI
- Prompt Engineering
- OCR

Deployment

- Lovable AI

---

# 💼 Skills Demonstrated

This project demonstrates practical experience in:

- Product Thinking
- Business Analysis
- AI Product Development
- Prompt Engineering
- OCR Integration
- Healthcare AI
- Responsible AI
- UX Design
- Privacy by Design
- Dashboard Design

---

# 🚧 Challenges Solved

During development I focused on solving several real-world challenges:

- Presenting medical information responsibly
- Preventing AI hallucinations
- Simplifying complex terminology
- Maintaining user privacy
- Designing a clean healthcare experience
- Building an intuitive AI chat interface

---

# 📈 Future Enhancements

- User Authentication
- Report History
- Multi-language Support
- Doctor Report Export
- Family Health Dashboard
- Download PDF Summary
- Voice Assistant
- AI Health Trends
- Smart Follow-up Recommendations

---

# 📸 Screenshots

## Home Screen

<img width="1025" height="1072" alt="Dashboard-ScreenShot" src="https://github.com/user-attachments/assets/c7fc20ad-428e-40ae-9521-58e24c910523" />


---

## Upload Medical Report

<img width="1542" height="1188" alt="upload-screen" src="https://github.com/user-attachments/assets/919ca31e-5a3e-482f-b5eb-7c9e639e2ed6" />


---

## AI Analysis Dashboard

<img width="987" height="963" alt="analysis-dashboard-3" src="https://github.com/user-attachments/assets/e7740f6b-306d-487b-a5f5-f65c510f8a34" />
<img width="1213" height="945" alt="analysis-dashboard-2" src="https://github.com/user-attachments/assets/0b328aa4-bb90-4d21-aac7-2e04e14d22e9" />
<img width="1038" height="975" alt="analysis-dashboard-1" src="https://github.com/user-attachments/assets/683b077f-aaa6-480c-98d9-fc70b04b0fc3" />


---

## AI Chat Assistant
<img width="1493" height="897" alt="ai-chat" src="https://github.com/user-attachments/assets/fae06744-eb32-40b6-858b-1eb092ca3604" />



---

# 📚 What I Learned

Building HealthInsight AI strengthened my knowledge of:

- AI Product Development
- Prompt Engineering
- Responsible AI
- Privacy-first Architecture
- OCR Workflows
- Healthcare UX
- AI-powered Dashboard Design
- End-to-End Product Development

---

# ⚠ Disclaimer

HealthInsight AI provides educational information only.

It is NOT intended to diagnose, treat, cure, or prevent any disease.

Users should always consult qualified healthcare professionals for medical advice.

---

# 👨‍💻 Author

**Rohit Pandey**

Business Operations Analyst • AI Product Builder • Business Analyst

LinkedIn:
https://linkedin.com/in/rohitpandey-insigh/

GitHub:
https://github.com/pandeykrohit-sys
